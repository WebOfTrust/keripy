...
from ...... import help
from ..... import indirecting, habbing, grouping, forwarding
...
class RegistryInceptor(doing.DoDoer):
    ...
    def __init__(self, name, base, alias, bran, registryName, nonce, estOnly, noBackers, baks, usage, toad=None, vcp=None, **kwa):
        ...
        Parameters:
            ...
            nonce (str): unique random value to seed the credential registry
            estOnly (bool): only allow establishment events for anchoring events
            noBackers (bool): do not allow setting up backers different from witnesses
            baks (list): new set of backers different from witnesses
            usage (str): message about how this registry is to be used
            toad (str): hex of witness threshold for the registry
    ...
    def inceptDo(self, tymth, tock=0.0, **kwa):
        ...
        registry_kwa = {
            'nonce': self.nonce,
            'noBackers': self.noBackers,
            'baks': self.baks,
            'estOnly': self.estOnly,
            'toad': self.toad,
            'vcp': self.vcp
        }
        registry = self.rgy.makeRegistry(name=self.registryName, prefix=hab.pre, **registry_kwa)
