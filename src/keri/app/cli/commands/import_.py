...
class ImportDoer(doing.DoDoer):
    ...
    def __init__(self, name, base, bran, cesrIn=None, **kwa):
        ...
        # lax settings are permissive for imported historical material.
        cues = []
    ...
    def importDo(self, tymth, tock=0.0, **kwa):
        ...
        # Refresh the Regery so named Registry objects are available without reopening
        # the same LMDB environment inside this process.
        self.rgy.setup()
        self._processEscrows("post-import")

        self._notice("import: completed after %d escrow passes. Target keystore should now reflect imported state.",
                     passes)
        return
