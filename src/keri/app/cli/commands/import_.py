...
class ImportDoer(doing.DoDoer):
    ...
    def importDo(self, tymth, tock=0.0, **kwa):
        ...
        ims = self._readCESR()  # pulls CESR stream from either .cesr files or stdin
        if not ims:
        ...
        # Pull everything in to the parser
        try:
            self.psr.parse(ims=bytearray(ims))
        except Exception as ex:
            logger.warning("import: parser raised during feed (continuing to escrows): %s", ex)

        # Due to the dependency chain of:
        #   KEL completion
        #     -> TEL anchor validation
        #       -> registry/credential TEL state
        #         -> ACDC verifier completion
        #           -> credential indexes
        # we must iterate through the escrows a few times to get everything to settle.
        # A single escrow pass does not work because the input stream is not perfectly ordered.
        #
        # We must assume the import bundle or stdin stream could be unordered, meaning events can
        # come out of order and thus we must process escrows a number of times for everything to
        # settle.
        #
        # For now this uses an ugly arbitrary count of 64 for looping escrow processing. Current
        # parser design of tight coupling  between parsing, event routing, and escrow handling
        # prevents extracting SAIDs from the event stream for discovering a list of events to wait
        # for complete ingestion of.
        maxPasses = 64
        passes = 0
        while passes < maxPasses:  # ugly arbitrary count of escrow passes, for now
            self._processEscrows(context="main")
            yield self.tock
            passes += 1

        # reads registries from files, or, when using stdin gets registries from Reger.
        registries = self._regsFromFiles()
        if not registries:
            registries = self._getRegistries()

        # waits for registries to appear in Reger.tevers before allowing command to complete
        yield from self._waitForRegistries(registries)

        # Imported registries MUST be named to be visible to `kli vc registry list` and other KLI commands.
        # We default registry names here to registry prefixes.
        self._saveDefaultRegNames(registries)

        self._notice("import: completed. Target keystore now contains imported state.")
        return

    def _readCESR(self):
        """
        Return a single bytes object containing all CESR to be imported.
        Loads from stdin if stdin, else reads all .cesr files in the --cesr-in path.
        """
        # stdin
        if not self.cesrIn:
    ...
    def _waitForRegistries(self, registries):
        ...
        while pending:
            ...
            self._processEscrows(context="registry wait")
            yield 0.05
            passes += 1
            if passes >= maxPasses:
                break
