...
class Baser(LMDBer):
    ...
    Attributes:
        ...
        Approval: via anchoring of the delegated event seal in the
        ...
        Key: said (bytes).
            ...
            .kramTRQS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'trqs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Prefixer, Number, Diger, Siger) tuple. Sourced from
                parser kwa key 'trqs'.
                Multiple values per key stored as ordered set (duplicates ignored).
        ...
        Value: (Prefixer, Number, Diger, Siger) tuple. Sourced from
            ...
            .kramSSCS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'sscs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Number, Diger) tuple. Sourced from parser kwa key 'sscs'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramSSTS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'ssts.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Prefixer, Number, Diger) tuple. Sourced from parser kwa
                key 'ssts'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramFRCS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'frcs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Number, Dater) tuple. Sourced from parser kwa key 'frcs'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramTDCS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'tdcs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Verser, Diger) tuple. Sourced from parser kwa key 'tdcs'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramPTDS is named subDB instance of IoSetSuber for KRAM partially signed
                ...
                subkey 'ptds.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: raw bytes of pathed CESR stream. Sourced from parser kwa
                key 'ptds'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramBSQS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'bsqs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Diger, Noncer, Noncer, Labeler) tuple. Sourced from
                parser kwa key 'bsqs'.
                Multiple values per key stored as ordered set (duplicates ignored).
                Entries persist until removed by the KRAM pruner.
            ...
            .kramBSSS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'bsss.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Diger, Noncer, Noncer, Labeler, Number, Noncer) tuple.
            ...
            .kramTMQS is named subDB instance of CatCesrIoSetSuber for KRAM partially
                ...
                subkey 'tmqs.'
        DB: keyed by (AID, MID): sender identifier prefix plus message SAID
        Value: (Diger, Noncer, Labeler, Texter) tuple. Sourced from
            parser kwa key 'tmqs'.
            Multiple values per key stored as ordered set (duplicates ignored).
            Entries persist until removed by the KRAM pruner.
    ...
    def clonePreIter(self, pre, fn=0, gvrsn=Vrsn_1_0, *, version=None):
        ...
        Parameters:
            pre: bytes of itdentifier prefix
            fn: int fn to resume replay. Earliset is fn=0
