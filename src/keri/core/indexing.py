...
from ..help import sceil, intToB64, b64ToInt, codeB64ToB2, codeB2ToB64, nabSextets
...
class IndexerCodex:
    ...
    ECDSA_256k1_Big_Crt_Sig: str = (
        "2D"  # ECDSA secp256k1 sig appears in current list only.
    )
    ECDSA_256r1_Big_Sig: str = "2E"  # ECDSA secp256r1 sig appears in both lists.
    ECDSA_256r1_Big_Crt_Sig: str = (
        "2F"  # ECDSA secp256r1 sig appears in current list only.
    )
...
class Indexer:
    ...
    def __init__(
        ...
        **kwa,
        ...
        """
        Validate material as fully qualified indexer data.
        ...
        Notes:
            Requires either ``raw`` with ``code`` and ``index``, or one of
            ``qb64b``, ``qb64``, or ``qb2``. Raises ``EmptyMaterialError`` when
            no usable input is provided.

            When ``raw`` is provided, validates code/size/index constraints and
            assigns ``.raw``. When qualified input is provided, extracts and
            assigns ``.raw``, ``.code``, ``.index``, and ``.ondex``.

        """
        if raw is not None:  # raw provided
            if not code:
                raise EmptyMaterialError(
                    "Improper initialization need either "
            ...
            if (
                isinstance(ondex, int)
                and os
                and not (ondex >= 0 and ondex <= (64**os - 1))
            ):
            ...
            if code in IdxBthSigDex:
                if ondex is None:  # set default
                    ...
                    if ondex != index and os == 0:  # must match if os == 0
                        raise InvalidVarIndexError(
                            f"Non matching ondex={ondex}"
            ...
            if not fs:  # compute fs from index
                if cs % 4:
                    raise InvalidCodeSizeError(
                        f"Whole code size not multiple of 4 for "
                        f"variable length material. cs={cs}."
                    )
                if os != 0:
                    raise InvalidCodeSizeError(
                        f"Non-zero other index size for "
            ...
            if len(raw) != rawsize:  # forbids shorter
                raise RawMaterialError(
                    f"Not enougth raw bytes for code={code}"
    ...
    def _infil(self):
        ...
        ps = (
            3 - (len(raw) % 3)
        ) % 3  # if lead then same pad size chars & lead size bytes
        ...
        if not fs:  # compute fs from index
            ...
            if os != 0:
                raise InvalidCodeSizeError(
                    f"Non-zero other index size for variable length material. os={os}."
                )
            fs = (index * 4) + cs
        ...
        if isinstance(ondex, int) and os and not (ondex >= 0 and ondex <= (64**os - 1)):
            raise InvalidVarIndexError(
                f"Invalid ondex={ondex} for os={os} and code={code}."
            )

        # both is hard code + converted index + converted ondex
        both = (
            f"{code}{intToB64(index, l=ms)}"
        ...
        if len(both) != cs:
            raise InvalidCodeSizeError(
                "Mismatch code size = {} with table = {}.".format(cs, len(both))
            )

        if (cs % 4) != ps - ls:  # adjusted pad given lead bytes
            raise InvalidCodeSizeError(
                f"Invalid code={both} for converted raw pad size={ps}."
            )
    ...
    def _exfil(self, qb64b):
        ...
        if first not in self.Hards:
            if first[0] == "-":
                raise UnexpectedCountCodeError(
                    "Unexpected count code startwhile extracing Indexer."
                )
            elif first[0] == "_":
                raise UnexpectedOpCodeError(
                    "Unexpected  op code startwhile extracing Indexer."
                )
            else:
    ...
    def _bexfil(self, qb2):
        ...
        if first not in self.Bards:
            if first[0] == b"\xf8":  # b64ToB2('-')
                raise UnexpectedCountCodeError(
                    "Unexpected count code startwhile extracing Matter."
                )
            elif first[0] == b"\xfc":  # b64ToB2('_')
                raise UnexpectedOpCodeError(
                    "Unexpected  op code startwhile extracing Matter."
                )
            else:
        ...
        if hard in IdxCrtSigDex:  # if current sig then ondex from code must be 0
            ondex = (
                b64ToInt(both[hs + ms : hs + ms + os]) if os else None
            )  # compute ondex from code
            if ondex:  # not zero or None so error
                raise ValueError(f"Invalid ondex={ondex} for code={hard}.")
            else:
                ondex = None  # zero so set to None when current only
        ...
        if ps:  # ps. IF ps THEN not ls (lead) and vice versa OR not ps and not ls
            ...
            if pi & (2**pbs - 1):  # masked pad bits non-zero
                raise ValueError(
                    f"Non zeroed pad bits = {pi & (2**pbs - 1):>08b} in 0x{pi:02x}."
                )
        else:  # not ps. IF not ps THEN may or may not be ls (lead)
...
class Siger(Indexer):
    ...
    def __init__(self, verfer=None, **kwa):
        ...
        if self.code not in IdxSigDex:
            raise ValidationError("Invalid code = {} for Siger.".format(self.code))
        self.verfer = verfer
