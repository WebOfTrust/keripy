...
def deReceiptCouple(data, strip=False):
    ...
    Parameters:
        data: couple of bytes concatenation of pre+sig from receipt
        strip: Boolean True means delete from data each part as parsed
...
def verifySigs(raw, sigers, verfers):
    ...
    Parameters:
        ...
        sigers: list of indexed Siger instances (signatures)
...
def interact(pre,
    ...
    Utility function to automate creation of interaction events.
     ...
     Parameters:
         pre: identifier prefix qb64
         dig: said digest of previous event qb64
         sn: int sequence number
         data: list of dicts of comitted data such as seals
         version (Versionage): KERI protocol default version if psvrsn is None
         pvrsn (Versionage): KERI protocol version
         gvrsn (Versionage): CESR genus vrsion
...
def receipt(pre,
    ...
    Utility function to automate creation of receipts.
     ...
     Parameters:
         pre: qb64 str of prefix of event being receipted
         sn: int sequence number of event being receipted
         said: qb64 of said of event being receipted
         version (Versionage): KERI protocol default version if psvrsn is None
         pvrsn (Versionage): KERI protocol version
         gvrsn (Versionage): CESR genus vrsion
         kind: serialization kind of receipt"""
    pvrsn = pvrsn if pvrsn is not None else version
    vs = versify(pvrsn=pvrsn, kind=kind, size=0, gvrsn=gvrsn)  # ensures cesr v2 only
...
def messagize(serder, *, sigers=None, tsgs=None, lsgs=None, wigers=None,
                         cigars=None, bonds=None, nests=None,
    ...
    if not (sigers or tsgs or lsgs or cigars or wigers or bonds):
...
class Kever:
    ...
    def __init__(self, *, state=None, serder=None, sigers=None, wigers=None,
        ...
        sigers, wigers, delpre, delsner, delsger = self.valSigsWigsDel(
                                                        serder=serder,
                                                        sigers=sigers,
    ...
    def incept(self, serder, estOnly=None):
        ...
        Parameters:
            ...
            estOnly: boolean  to indicate establish only events allowed"""
        ked = serder.ked
    ...
    def rotate(self, serder):
        ...
        if sner.num > self.sner.num + 1:  # out of order event
            ...
            if ((ilk == Ilks.rot and sner.num <= self.lastEst.s) or
                ...
                pdig = self.db.kels.getLast(keys=pre, on=psn)
                if pdig is None:
    ...
    def valSigsWigsDel(self, serder, sigers, verfers, tholder,
                                wigers, toader, wits, *,
                                delsner=None, delsger=None, eager=False,
                                local=True):
        """
        ...
        if (not local and (self.locallyOwned() or
            ...
            self.escrowMFEvent(serder=serder, sigers=sigers, wigers=wigers,
                               delsner=delsner, delsger=delsger, local=local)
        ...
        if (serder.ilk in (Ilks.dip, Ilks.drt) and self.locallyDelegated(delpre)
            ...
            if delsner is None or delsger is None: # missing delegation seal
        ...
        # does not validate.
        delsner, delsger = self.validateDelegation(serder,
                                                        sigers=sigers,
                                                        wigers=wigers,
                                                        wits=wits,
                                                        delpre=delpre,
        ...
        return (sigers, wigers, delpre, delsner, delsger)
    ...
    def validateDelegation(self, serder, sigers, wigers, wits, delpre, *,
                    delsner=None, delsger=None, eager=False, local=True):
        """
        ...
        Returns:
            ...
            it: received and authenticated on it its local (protected) channel
        ...
        if delsner is None or delsger is None: # missing delegation seal ref
            if eager:  # walk kel here to find
                ...
                if dserder is not None:  # found seal in dserder
                    delsner = Number(num=dserder.sn)  # replace with found
                    delsger = Diger(qb64=dserder.said)  # replace with found
        ...
        if delsner and delsger and not dserder:  # given couple not found
            # Get delegating event from delnum and delpre
            ssn = delsner.validate(inceptive=False).sn
            ...
            raw = self.db.kels.getLast(keys=delpre, on=ssn)  # last means not disputed or superseded
            if raw is None:  # no delegatint event yet. no index at key pre, sn
                ...
                self.escrowPDEvent(serder=serder, sigers=sigers, wigers=wigers,
                                   delsner=delsner, delsger=delsger, local=local)
                msg = (f"No delegating event from {delpre} at {delsger.qb64} for evt "
                       f"{serder.sn} {serder.ilk} {serder.said}")
                logger.info(msg)
                logger.debug("Event Body=\n%s\n", serder.pretty())
                raise MissingDelegationError(msg)
            ...
            delsner = Number(numh=dserder.snh)  # replace with found
            delsger = Diger(qb64=dserder.said)  # replace with found
        ...
        if ((serder.ilk == Ilks.dip) or  # superseding is delegated inception or
            ...
            (serder.sner.num == self.sner.num and  # superseding event at same sn and
                ...
                serder.ilk == Ilks.drt)):  # superseding is rotation
                    return (delsner, delsger) # indicates delegation valid
    ...
    def fetchDelegatingEvent(self, delpre, serder, *, original=True, eager=False):
        ...
        if (duple := self.db.aess.get(keys=(serder.preb, serder.saidb))):  # delegation source couple at delegate
            ...
            if self.db.fons.get(keys=(serder.pre, serder.said)):
                ...
                self.db.aess.pin(keys=(serder.preb, serder.saidb), val=(Number(num=dserder.sn), diger))  # authorizer (delegator/issuer) event seal
    ...
    def logEvent(self, serder, sigers=None, wigers=None, wits=None, first=False,
        ...
        Parameters:
            serder: SerderKERI instance of current event
            sigers: optional list of Siger instance for current event
            wigers: optional list of Siger instance of indexed witness sigs
            wits: optional list of current witnesses provide during any establishment event
            first: Boolean True means first seen accepted log of event.
                Otherwise means idempotent log of event to accept additional
                signatures beyond the threshold provided for first seen
            delnum: Number instance of delegating event sequence number.
                If this event is not delegated then delnum is ignored
            diger: Diger instance of of delegating event said.
                If this event is not delegated then diger is ignored
        ...
        if (self.delpre and not serder.ilk == Ilks.ixn and not self.locallyOwned()
            and not self.locallyWitnessed(wits=wits) and delnum and diger):
            self.db.aess.pin(keys=(serder.preb, serder.saidb), val=(Number(num=delnum.num, code=NumDex.Huge), diger))  # authorizer (delegator/issuer) event seal
        ...
        if first:  # append event dig to first seen database in order
            ...
            logger.debug("Event Body=\n%s\n", serder.pretty())
        self.db.kels.add(keys=serder.preb, on=serder.sn, val=serder.saidb)
    ...
    def escrowMFEvent(self, serder, sigers, wigers=None,
                      delsner=None, delsger=None, local=True):
        """
        ...
        if delsner and delsger:
            self.db.udes.put(keys=dgkey, val=(delsner, delsger))
    ...
    def escrowPSEvent(self, serder, *, sigers=None, wigers=None,
        ...
        self.db.pses.add(keys=serder.preb, on=serder.sn, val=serder.saidb)
    ...
    def escrowPWEvent(self, serder, *, sigers=None, wigers=None,
        ...
        Parameters:
            serder: SerderKERI instance of  event
            sigers: optional list of Siger instances of indexed controller sigs
            wigers: list of Siger instance of indexed witness sigs
            delsner (Number): instance of sn of delegating event if any
            delsger (Diger): instance of said digest of delegating event if any
            local (bool): event source for validation logic
                True means event source is local (protected).
        ...
        logger.trace("Event Body=\n%s\n", serder.pretty())
        return self.db.pwes.add(keys=serder.preb, on=serder.sn, val=serder.saidb)


    def escrowPDEvent(self, serder, *, sigers=None, wigers=None,
                      delsner=None, delsger=None, local=True):
        """Update associated logs for escrow of partially delegated event.
        ...
        Parameters:
            ...
            sigers: optional list of Siger instances of indexed controller sigs
            wigers: list of Siger instance of indexed witness sigs
            delsner: Number instance of sn of seal source delegating event sn
            delsger: Diger instance of digest of delegating event said
            local (bool): event source for validation logic
                True means event source is local (protected).
                False means event source is remote (unprotected).
                Event validation logic is a function of local or remote"""
        ...
        if wigers:  # idempotent
            self.db.wigs.put(keys=dgkey, vals=wigers)
        if delsner and delsger:  # non-idempotent pin to repair replace
            self.db.udes.pin(keys=dgkey, val=(delsner, delsger))  # non-idempotent
            logger.debug(f"Kever state: Replaced escrow source couple sn="
                         f"{delsner.num}, said={delsger.qb64} for partially "
    ...
    def fetchPriorDigers(self, sn: int | None = None) -> list | None:
        ...
        for digb in self.db.kels.getBackIter(keys=pre, on=sn):
    ...
    def fetchLatestContribTo(self, verfers, sn: int | None = None):
        ...
        Returns:
            ...
            sn: sequence number.
            idx: index of verfer in verfers

            verfer: instance of Verfer
    ...
    def fetchLatestContribFrom(self, verfer, sn: int | None = None):
        ...
        Returns:
            ...
            sn: sequence number
            index: index into verfers of verfers

            verfers: list of Verfer instances.
...
class Kevery:
    ...
    def fetchWitnessState(self, pre, sn):
        ...
        for digb in self.db.kels.getBackIter(keys=preb, on=sn):
        ...
        if pre not in self.kevers:  # first seen event for pre
            ...
            if ilk in (Ilks.icp, Ilks.dip):  # another inception event so maybe duplicitous
                ...
                if sn > sno:  # sn later than sno so out of order escrow
                    ...
                    ddig = self.db.kels.getLast(keys=pre, on=sn)
    ...
    def processAttachedReceiptCouples(self, serder, cigars, *, firner=None,
        ...
        if firner:
            ldig = self.db.fels.get(keys=pre, on=firner.sn)
        else:
            ldig = self.db.kels.getLast(keys=pre, on=sn)  # retrieve dig of last event at sn.
    ...
    def processAttachedReceiptSigGroups(self, serder, tsgs, *, firner=None,
                                         local=None, **kwa):
        ...
        This is the attachment version of .processReceiptTrans

        Parameters:
            serder (serderKERI):  instance serialized event message to which
                attachments come from replay (clone)
            tsgs (list[tuple]):  (prefixer, number, diger, [sigers])
            firner (Seqner): instance of first seen ordinal,
                if provided lookup event by fn = firner.sn
                used when in cloned replay mode
            local (bool|None): True means local (protected) event source.
        ...
        for sprefixer, snumber, sdiger, sigers in tsgs:  # iterate over each trq
            if not self.lax and sprefixer.qb64 in self.prefixes:  # own trans idx sig (chit)
                if pre in self.prefixes:  # skip own trans idx sig endorsement of own events
                    raise ValidationError("Own pre={} replay of attached transferable "
                                          "idx sig of own event {}."
                                          "".format(self.prefixes, serder.pretty()))
                if not local:  # skip own trans idx sig endorsement of nonlocal events
                    raise ValidationError("Own pre={} in replay of attached "
                                          "transferable idx sig of nonlocal"
    ...
    def removeStaleReplyEndRole(self, saider):
        ...
        Parameters:
            saider (Saider): SAID of the reply SAD whose escrow entry should
                be removed.
        """
        pass
    ...
    def fetchEstEvent(self, pre, sn):
        ...
        Parameters:
            pre: qb64 of identifier prefix for KEL
            sn: int sequence number of event in KEL of pre"""
    ...
    def escrowMFEvent(self, serder, sigers, wigers=None,
                      number=None, diger=None, local=True):
        """
        Update associated logs for escrow of MisFit event
    ...
    def escrowUWReceipt(self, serder, wigers, said):
        ...
        for wiger in wigers:  # escrow each couple
            ...
            self.db.uwes.add(keys=serder.preb, on=serder.sn, val=(said, wiger.qb64))
    ...
    def escrowUReceipt(self, serder, cigars, said):
        ...
        Parameters:
            ...
            serder: of receipt not receipted event"""
        # note receipt dig algo may not match database dig also so must always
        # serder.compare to match. So receipts for same event may have different
        # digs of that event due to different algos. So the escrow may have
        # different dup at same key, sn.  Escrow needs to include dig
        ...
        for pre, sn, edig in self.db.ooes.getAllItemIter():
            ...
            try:
                ...
                if dater is None:  # no datetime stored
                    ...
                    msg = f"OOO Missing escrowed event datetime at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutOOE):
                    ...
                    msg = f"OOO Stale event escrow at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (eserder := self.db.evts.get(keys=(pre, bytes(edig)))) is None:
                    ...
                    msg = f"OOO Missing escrowed event at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if not sigers:  # otherwise its a list of sigs
                    ...
                    msg = f"OOO Missing escrowed event sigs at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                self.db.ooes.rem(keys=pre, on=sn, val=edig)  # removes one escrow at key val
                if logger.isEnabledFor(logging.DEBUG):
    ...
    def processEscrowPartialSigs(self):
        ...
        for pre, sn, edig in self.db.pses.getAllItemIter():
            ...
            try:
                ...
                if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                    # no local source so raise ValidationError which unescrows below
                    msg = f"PSE Missing escrowed event source at dig = {edig.decode()}"
                ...
                if dater is None:  # no datetime stored
                    # no date time so raise ValidationError which unescrows below
                    msg = f"PSE Missing escrowed event datetime at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutPSE):
                    # escrow stale so raise ValidationError which unescrows below
                    msg = f"PSE Stale event escrow at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (eserder := self.db.evts.get(keys=(pre, bytes(edig)))) is None:
                    ...
                    msg = f"PSE Missing escrowed evt at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                #  get sigs and attach
                sigers = self.db.sigs.get(keys=(pre, edig))
                if not sigers:  # otherwise its a list of sigs
                    # no sigs so raise ValidationError which unescrows below
                    msg = f"PSE Missing escrowed evt sigs at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                wigers = self.db.wigs.get(keys=(pre, bytes(edig)))
                ...
                self.db.pses.rem(keys=pre, on=sn, val=edig)  # removes one escrow at key val
    ...
    def processEscrowPartialWigs(self):
        ...
        for pre, sn, edig in self.db.pwes.getAllItemIter(keys=b''):
            try:
                ...
                if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                    # no local source so raise ValidationError which unescrows below
                    msg = f"PWE Missing escrowed event source at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if dater is None:  # no datetime stored
                    # no date time so raise ValidationError which unescrows below
                    msg = f"PWE Missing escrowed event datetime at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutPWE):
                    # escrow stale so raise ValidationError which unescrows below
                    msg = f"PWE Stale event escrow at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (eserder := self.db.evts.get((pre, bytes(edig)))) is None:
                    # no event so so raise ValidationError which unescrows below
                    msg = f"PWE Missing escrowed evt at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if not sigers:  # empty list
                    # no sigs so raise ValidationError which unescrows below
                    msg = f"PWE Missing escrowed evt sigs at dig = {edig.decode()}"
                    logger.trace("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                self.db.pwes.rem(keys=pre, on=sn, val=edig)  # removes one escrow at key val
                #self.db.udes.rem(keys=dgkey)  # leave here since could PartialDelegationEscrow
                if logger.isEnabledFor(logging.TRACE):
                ...
                self.db.pwes.rem(keys=pre, on=sn, val=edig)  # removes one escrow at key val
                self.db.udes.rem(keys=dgkey)  # remove escrow if any
                logger.info("Kevery: PWE unescrow succeeded in valid event: key = %s \tdigest = %s",
                           pre, sn, edig.decode())
                logger.debug("Event=\n%s\n", eserder.pretty())
    ...
    def processEscrowPartialDels(self):
        ...
        for (epre,), esn, edig in self.db.pdes.getAllItemIter(keys=b''):
            try:
                ...
                if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                    # no local source so raise ValidationError which unescrows below
                    msg = f"PDE Missing escrowed event source at dig = {edig}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if dater is None:  # no datetime stored
                    # no date time so raise ValidationError which unescrows below
                    msg = f"PDE Missing escrowed event datetime at dig = {edig}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutPWE):
                    # escrow stale so raise ValidationError which unescrows below
                    msg = f"PDE Stale event escrow at dig = {edig}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (eserder := self.db.evts.get(keys=(epre, edig))) is None:
                    # no event so so raise ValidationError which unescrows below
                    msg = f"PDE Missing escrowed evt at dig = {edig}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                # self.db.pdes.addOn(pre, sn, serder.digb)
                # which in turn will NOT enter dig as dup if one already exists.
                ...
                self.db.pdes.rem(keys=epre, on=esn, val=edig)  # event idx escrow
                ...
                self.db.pdes.rem(keys=epre, on=esn, val=edig)  # event idx escrow
                self.db.udes.rem(keys=dgkey)  # remove source seal escrow if any
                logger.info("Kevery PDE unescrow succeeded in valid event: "
                            "event=%s", eserder.said)
                logger.debug("Event=\n%s\n", eserder.pretty())
    ...
    def processEscrowUnverWitness(self):
        ...
        for (pre, ), sn, (rdig, wig) in self.db.uwes.getTopItemIter():
            try:
                ...
                # check date if expired then remove escrow.
                dater = self.db.dtss.get(keys=dgKey(pre, rdig))
                if dater is None:  # no datetime stored
                    # no date time so raise ValidationError which unescrows below
                    msg = f"UWE Missing escrowed event datetime at dig = {rdig}"
                    logger.trace("Kevery unescrow error: %s", rdig)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutUWE):
                    # escrow stale so raise ValidationError which unescrows below
                    msg = f"UWE Stale event escrow at dig = {rdig}"
                    logger.trace("Kevery unescrow error: %s", rdig)
                    raise ValidationError(msg)
                ...
                #sn = int(snh, 16)
                rdiger = Diger(qb64=rdig)
                wiger = Siger(qb64=wig)
                ...
                if not found:  # no partial witness escrow of event found
                    ...
                    msg = f"UWE Missing witness receipted evt at pre={pre} {sn=}"
                    logger.trace("Kevery unescrow error: %s", msg)
                ...
                # error other than out of order so remove from OO escrow
                self.db.uwes.rem(keys=(pre,), on=sn, val=(rdig, wig))
                if logger.isEnabledFor(logging.DEBUG):  # adds exception data
    ...
    def processEscrowDelegables(self):
        ...
        for (pre, sn), dig in self.db.delegables.getTopItemIter():
            try:
                ...
                if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                    # no local source so raise ValidationError which unescrows below
                    msg = f"DEL Missing escrowed event source at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if dater is None:  # no datetime stored
                    # no date time so raise ValidationError which unescrows below
                    msg = f"DEL Missing escrowed event datetime at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutOOE):
                    # escrow stale so raise ValidationError which unescrows below
                    msg = f"DEL Stale event escrow at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if (eserder := self.db.evts.get(keys=(pre, bytes(edig)))) is None:
                    # no event so raise ValidationError which unescrows below
                    msg = f"DEL Missing escrowed evt at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
                ...
                if not sigers:  # otherwise its a list of sigs
                    # no sigs so raise ValidationError which unescrows below
                    msg = f"DEL Missing escrowed evt sigs at dig = {edig.decode()}"
                    logger.info("Kevery unescrow error: %s", msg)
                    raise ValidationError(msg)
        ...
        while True:  # break when done
            for (pre, said), edig in self.db.qnfs.getTopItemIter(keys=key):
                try:
                    ...
                    if dater is None:  # no datetime stored
                        # no date time so raise ValidationError which unescrows below
                        msg = f"QNF Missing escrowed event datetime at dig = {edig}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutQNF):
                        # escrow stale so raise ValidationError which unescrows below
                        msg = f"QNF Stale qry event escrow at dig = {edig}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if (eserder := self.db.evts.get(keys=(pre.encode("utf-8"), edig.encode("utf-8")))) is None:
                        # no event so raise ValidationError which unescrows below
                        msg = f"QNF Missing escrowed evt at dig = {edig}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if not sigers:  # otherwise its a list of sigs
                        # no sigs so raise ValidationError which unescrows below
                        msg = f"QNF Missing escrowed evt sigs at dig = {edig}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if logger.isEnabledFor(logging.TRACE):
                        ...
                        logger.exception("Kevery: QNF unescrow failed: %s\n", ex.args[0])
    ...
    def _processEscrowFindUnver(self, pre, sn, rsaider, wiger=None, cigar=None):
        ...
        found = False
        for dig in self.db.pwes.getIter(keys=pre, on=sn):  # search entries
        ...
        while True:  # break when done
            for ekey, equinlet in self.db.vres.getTopItemIter(keys=key):
                try:
                    ...
                    raw = self.db.kels.getLast(keys=pre, on=sn)
                    if raw is None:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):  # adds exception data
                        ...
                        logger.exception("Kevery: VRE other error on unescrow: %s\n", ex.args[0])
        ...
        while True:  # break when done
            for (pre,), sn, edig in self.db.ldes.getAllItemIter(keys=key):
                try:
                    ...
                    if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                        # no local source so raise ValidationError which unescrows below
                        msg = f"DUP Missing escrowed event source at dig = {edig.decode()}"
                        logger.info("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if dater is None:  # no datetime stored
                        # no date time so raise ValidationError which unescrows below
                        msg = f"DUP Missing escrowed event datetime at dig = {edig.decode()}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutLDE):
                        # escrow stale so raise ValidationError which unescrows below
                        msg = f"DUP Stale event escrow at dig = {edig.decode()}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if (eserder := self.db.evts.get(keys=(pre, bytes(edig)))) is None:
                        # no event so raise ValidationError which unescrows below
                        msg = f"DUP Missing escrowed evt at dig = {edig.decode()}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if not sigers:  # otherwise its a list of sigs
                        # no sigs so raise ValidationError which unescrows below
                        msg = f"DUP Missing escrowed evt sigs at dig = {edig.decode()}"
                        logger.trace("Kevery unescrow error: %s", msg)
                        raise ValidationError(msg)
                    ...
                    if logger.isEnabledFor(logging.TRACE):
                        ...
                        logger.exception("Kevery: DUP unescrow failed: %s\n", ex.args[0])

                except Exception as ex:  # log diagnostics errors etc
                    # error other than likely duplicitous so remove from escrow
                    self.db.ldes.rem(keys=pre, on=sn, val=edig)  # removes one escrow at key val
                    if logger.isEnabledFor(logging.DEBUG):
...
def loadEvent(db, preb, dig):
    ...
    sn = serder.sn
    sdig = db.kels.getLast(keys=preb, on=sn)
    if sdig is not None:
        event["stored"] = True
