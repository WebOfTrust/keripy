...
class Tever:
    ...
    def reload(self, rsr):
        ...
        if (raw := self.reger.tvts.get(keys=dgKey(pre=self.prefixer.qb64,
                  dig=ked['d']))) is None:
    ...
    def revoke(self, serder, seqner, saider, sn, bigers=None):
        ...
        ievt = self.reger.tvts.get(keys=dgKey(pre=vci, dig=dig))
        if ievt is None:
...
class Tevery:
    ...
    def processReplyRegistryTxnState(self, *, serder, saider, route, cigars=None, tsgs=None, **kwargs):
        ...
        sraw = self.reger.tvts.get(keys=dgKey(pre=regk, dig=ldig))
    ...
    def processReplyCredentialTxnState(self, *, serder, saider, route, cigars=None, tsgs=None, **kwargs):
        ...
        ldig = bytes(ldig)
        # retrieve last event itself of signer given sdig
        sraw = self.reger.tvts.get(keys=dgKey(pre=vci, dig=ldig))
        # assumes db ensures that sraw must not be none because sdig was in KE
        sserder = serdering.SerderKERI(raw=sraw.encode("utf-8"))
