...
resolve_demo_witnesses() {
  local KLI_NAME=$1

  kli oobi resolve --name "${KLI_NAME}" --base "${BASE}" --oobi-alias wan --oobi "http://127.0.0.1:5642/oobi/${WAN}/controller"
  kli oobi resolve --name "${KLI_NAME}" --base "${BASE}" --oobi-alias wil --oobi "http://127.0.0.1:5643/oobi/${WIL}/controller"
  kli oobi resolve --name "${KLI_NAME}" --base "${BASE}" --oobi-alias wes --oobi "http://127.0.0.1:5644/oobi/${WES}/controller"
}
...
init_keystore() {
  ...
  kli init --name "${KLI_NAME}" --base "${BASE}" --salt "${SALT}" --nopasscode
  # Resolve explicitly so the workflow does not block inside init's config OOBI wait loop.
  resolve_demo_witnesses "${KLI_NAME}"
}

# Three multisig participants, though multisig starts only with 2 up front.
init_keystore multisig1 0ACDEyMzQ1Njc4OWxtbm9aBc
kli incept --name multisig1 --base "${BASE}" --alias multisig1 --file "${KERI_DEMO_SCRIPT_DIR}/data/multisig-1-sample.json"

init_keystore multisig2 0ACDEyMzQ1Njc4OWdoaWpsaw
kli incept --name multisig2 --base "${BASE}" --alias multisig2 --file "${KERI_DEMO_SCRIPT_DIR}/data/multisig-2-sample.json"

init_keystore multisig3 0ACDEyMzQ1Njc4OWdoaWpsaw
...
init_keystore holder 0ACDEyMzQ1Njc4OWxtbm9qWc
