...
kli init --name delegate1 --salt 0AMDEyMzQ1Njc4OWxtbm9wcQ --nopasscode --config-dir ./scripts --config-file demo-witness-oobis
kli incept --name delegate1 --alias delegate1 --file tests/app/cli/commands/delegate/delegate-1.json

echo "Creating delegate's second local identifier in delegate2 keystore"
kli init --name delegate2 --salt 0AMDEyMzQ1Njc4OWdoaWpsaw --nopasscode --config-dir ./scripts --config-file demo-witness-oobis
kli incept --name delegate2 --alias delegate2 --file tests/app/cli/commands/delegate/delegate-2.json

echo "Creating delegator's first local identifier in delegator1 keystore"
kli init --name delegator1 --nopasscode --config-dir ./scripts --config-file demo-witness-oobis --salt 0AMDEyMzQ1Njc4OWdoaWpdo1
kli incept --name delegator1 --alias delegator1 --file tests/app/cli/commands/delegate/delegator-1.json

echo "Creating delegator's second local identifier in delegator2 keystore"
kli init --name delegator2 --nopasscode --config-dir ./scripts --config-file demo-witness-oobis --salt 0AMDEyMzQ1Njc4OWdoaWpdo2
