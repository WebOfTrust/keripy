...
def test_tever_escrow(mockCoringRandomNonce):
    ...
    with basing.openDB() as db, keeping.openKS() as kpr, viring.openReger() as reg:
        ...
        anc = reg.ancs.get(keys=dgkey)
        assert anc.encode("utf-8") == b'0AAAAAAAAAAAAAAAAAAAAAABEErlKvdXWTpOg9s5AasfABIk17o_EaM2uAuGe-c_LEbc'
...
def test_tever_no_backers(mockHelpingNowUTC, mockCoringRandomNonce):
    ...
    with basing.openDB() as db, keeping.openKS() as kpr, viring.openReger() as reg:
        ...
        assert reg.ancs.get(keys=dgkey).encode("utf-8") == b'0AAAAAAAAAAAAAAAAAAAAAABEGe5uFh3t0JglSPQtJJoxCV1RlFoTBept1BPRk3o6hgh'
        ...
        assert bytes(reg.getTvt(dgkey)) == (
            ...
            b'0+00:00"}')
        assert reg.ancs.get(keys=dgkey).encode("utf-8") == b'0AAAAAAAAAAAAAAAAAAAAAADEHzX4VXW_xCDf_pFhFvkDdc6TXTxiSiaEpHHCr1tDVtD'
...
def test_tever_backers(mockHelpingNowUTC, mockCoringRandomNonce):
    ...
    with basing.openDB() as db, keeping.openKS() as kpr, viring.openReger() as reg:
        ...
        assert reg.ancs.get(keys=dgkey).encode("utf-8") == b'0AAAAAAAAAAAAAAAAAAAAAABEDD2vrz4Eg3iLOOlZw5-d3ioZ1q703IC0M0LJP_3v-PT'
