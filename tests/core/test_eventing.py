...
def test_dequadruple():
    ...
    with pytest.raises(TypeError):  # immutable so no strip delete
        sprefixer, sseqner, sdiger, siger = deTransReceiptQuadruple(
            quadruple, strip=True
        )
...
def test_dequintuple():
    ...
    assert len(quintuple) == 244
    with pytest.raises(TypeError):
        ediger, sprefixer, sseqner, sdiger, siger = deTransReceiptQuintuple(
            quintuple, strip=True
        )
    ...
    quintuple = bytearray(quintuple)
    assert len(quintuple) == 244
    ediger, sprefixer, sseqner, sdiger, sigar = deTransReceiptQuintuple(
        quintuple, strip=True
    )
    assert ediger.qb64b == edig.encode("utf-8")
    assert sprefixer.qb64b == spre.encode("utf-8")
...
def test_seals_states():
    ...
    seal = SealBack(bi="B4321", d="EABCDE")
    assert isinstance(seal, SealBack)
...
def test_keyeventfuncs(mockHelpingNowUTC):
    ...
    # print()
    seed = (
        b"\x9f{\xa8\xa7\xa8C9\x96&\xfa\xb1\x99\xeb\xaa \xc4\x1bG\x11\xc4\xaeSAR"
        b"\xc9\xbd\x04\x9d\x85)~\x93"
    )

    # Inception: Non-transferable (ephemeral) case
    signer0 = Signer(
        raw=seed, transferable=False
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON0000fd_","t":"icp","d":"EMW0zK3bagYPO6gx3w7Ua90f-I7x5kGIaI4X'
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON0000fd_","t":"icp","d":"EPLRRJFe2FHdXKVTkSEX4xb4x-YaPFJ2Xds1'
    ...
    # seed = pysodium.randombytes(pysodium.crypto_sign_SEEDBYTES)
    seed1 = (
        b"\x83B~\x04\x94\xe3\xceUQy\x11f\x0c\x93]\x1e\xbf\xacQ\xb5\xd6Y^\xa2E\xfa\x015"
        b"\x98Y\xdd\xe8"
    )
    ...
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012b_","t":"icp","d":"EAKCxMOuoRzREVHsHCkLilBrUXTvyenBiuM2'
    ...
    assert nxt1 == ["EIf-ENw7PrM52w4H-S7NGU2qVIfraXVIlV9hEAaMHg7W"]
    serder0 = incept(
        keys=keys0, ndigs=nxt1, code=MtrDex.Blake3_256, intive=True
    )  # intive true
    pre = serder0.ked["i"]
    assert serder0.ked["t"] == Ilks.icp
    ...
    assert serder0.raw == (
        b'{"v":"KERI10JSON000125_","t":"icp","d":"EIflL4H4134zYoRM6ls6Q086RLC_BhfNFh5u'
    ...
    assert serder0.raw == (
        b'{"v":"KERI10JSON000125_","t":"icp","d":"EFSJqZE0K0WU95dmccrg_8EKSuVSrt4kGIZN'
    ...
    assert serder0.ked["bt"] == "0"  # hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012b_","t":"icp","d":"EJQUyxnzIAtmZPoq9f4fExeGN0qfJmaFnUEK'
    ...
    # seed = pysodium.randombytes(pysodium.crypto_sign_SEEDBYTES)
    seed2 = (
        b"\xbe\x96\x02\xa9\x88\xce\xf9O\x1e\x0fo\xc0\xff\x98\xb6\xfa\x1e\xa2y\xf2"
    ...
    assert serder1.raw == (
        b'{"v":"KERI10JSON000160_","t":"rot","d":"EFl8nvRCbN2xQJI75nBXp-gaXuHJw8zheVjw'
    ...
    # compute nxt digest
    serder1 = rotate(
        pre=pre, keys=keys1, dig=serder0.said, ndigs=keys2, sn=1, intive=True
    )  # intive
    assert serder1.ked["t"] == Ilks.rot
    assert serder1.ked["i"] == pre
    assert serder1.ked["s"] == "1"
    ...
    assert serder1.raw == (
        b'{"v":"KERI10JSON00015a_","t":"rot","d":"ECauhEzA4DJDXVDnNQiGQ0sKXa6sx_GgS8Eb'
    ...
    assert serder2.raw == (
        b'{"v":"KERI10JSON0000cb_","t":"ixn","d":"EKKccCumVQdgxvsrSXvuTtjmS28Xqf3zRJ8T'
    ...
    assert serder3.raw == (
        b'{"v":"KERI10JSON000091_","t":"rct","d":"EKKccCumVQdgxvsrSXvuTtjmS28Xqf3zRJ8T'
    ...
    assert msg == (
        b'{"v":"KERI10JSON000091_","t":"rct","d":"EKKccCumVQdgxvsrSXvuTtjm'
    ...
    assert serderD.raw == (
        b'{"v":"KERI10JSON00015f_","t":"dip","d":"EN3PglLbr4mJblS4dyqbqlpUa735hVmLOhYU'
    ...
    assert serderR.raw == (
        b'{"v":"KERI10JSON000160_","t":"drt","d":"EMBBBkaLV7i6wNgfz3giib2ItrHsr548mtIf'
    ...
    signer0 = Signer(
        raw=seed, transferable=False, code=MtrDex.ECDSA_256r1_Seed
    )  # original signing keypair non transferable
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON000105_","t":"icp","d":"ELIz2CFNp4vCTJkCKYzqkv1tJeqaPiwhHkNuWA0tKfxo",'
    ...
    # Inception: Transferable Case but abandoned in incept so equivalent
    signer0 = Signer(
        raw=seed, code=MtrDex.ECDSA_256r1_Seed
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON000105_","t":"icp","d":"EPqQeDE6eoawHEwQjyB4kwLTwZ2VV6jDz_TXWFV7sE8T",'
    ...
    signer1 = Signer(
        raw=seed1, code=MtrDex.ECDSA_256r1_Seed
    ...
    assert (
        serder0.ked["d"]
        == serder0.ked["i"]
        == "EFEscYZrbSsAPJq_OhGt19qb-ci1AtZTqwGqZ5FypKVd"
    )
    assert serder0.ked["s"] == "0"
    assert serder0.ked["kt"] == "1"
    assert serder0.ked["nt"] == "1"
    ...
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012f_","t":"icp","d":"EFEscYZrbSsAPJq_OhGt19qb-ci1AtZTqwGqZ5FypKVd",'
    ...
    assert serder0.ked["bt"] == 0
    assert serder0.raw == (
        b'{"v":"KERI10JSON000129_","t":"icp","d":"EJcQxvzMr3xaxa6rlXvPguII45JMmoRENnKFAsUS9Mx0",'
    ...
    assert serder0.ked["bt"] == 0  # int not hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012d_","t":"icp","d":"ECMsDN8WUWcKGe7X0GNrecOgtrTkuHTfoo7YoQ4TIOLS",'
    ...
    assert serder0.ked["bt"] == "0"  # hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON000133_","t":"icp","d":"EF8tNYXNKPjByUNg2s7zEhEKKl39COxwOYA1CpqvxB_J",'
        b'"i":"1AAJA3cK_P2CDlh-_EMFPvyqTPI1POkw-dr14DANx5JEXDCZ","s":"0",'
        b'"kt":"1","k":["1AAJA3cK_P2CDlh-_EMFPvyqTPI1POkw-dr14DANx5JEXDCZ"],'
        b'"nt":"1","n":["EDCWQzPSj3zZBKMZ-_FAckxIMFM25ITsEwD72psBYak4"],'
        b'"bt":"0","b":[],"c":[],"a":[]}'
    ...
    signer2 = Signer(
        raw=seed2, code=MtrDex.ECDSA_256r1_Seed
    )  # next signing keypair transferable is default
    ...
    assert serder1.ked["bt"] == "0"  # hex str
    assert serder1.raw == (
        b'{"v":"KERI10JSON000168_","t":"rot","d":"ENsyn8FRcmd9N3Dhi_kRdWcnI_AIa7yhDNsHXycclSXQ",'
    ...
    signer0 = Signer(
        raw=seed, transferable=False, code=MtrDex.ECDSA_256k1_Seed
    )  # original signing keypair non transferable
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON000105_","t":"icp","d":"EGEP0h6tTUUOeIK4ApGlnLl2lwD0lbaQGBfL9'
    ...
    # Inception: Transferable Case but abandoned in incept so equivalent
    signer0 = Signer(
        raw=seed, code=MtrDex.ECDSA_256k1_Seed
    )  # original signing keypair transferable default
    assert signer0.code == MtrDex.ECDSA_256k1_Seed
    ...
    assert serder.ked["n"] == []
    assert serder.raw == (
        b'{"v":"KERI10JSON000105_","t":"icp","d":"EO3M4d4pvQu2SXFLaXEy05ey80d71gbEakA2TgCTnJtN",'
    ...
    signer1 = Signer(
        raw=seed1, code=MtrDex.ECDSA_256k1_Seed
    )  # next signing keypair transferable is default
    ...
    assert (
        serder0.ked["d"]
        == serder0.ked["i"]
        == "EBTJs_972Mh0Q2raFOINbmgtdtT0Od4VJm6aNd4xVW9u"
    )
    assert serder0.ked["s"] == "0"
    assert serder0.ked["kt"] == "1"
    assert serder0.ked["nt"] == "1"
    ...
    assert serder0.ked["bt"] == "0"  # hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012f_","t":"icp","d":"EBTJs_972Mh0Q2raFOINbmgtdtT0Od4VJm6aNd4xVW9u",'
    ...
    assert serder0.ked["bt"] == 0
    assert serder0.raw == (
        b'{"v":"KERI10JSON000129_","t":"icp","d":"ECzQWBHMIRJpUhrIB2sn4YUsb0HL-wE1wErVcQnkme5z",'
    ...
    assert serder0.ked["bt"] == 0  # int not hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON00012d_","t":"icp","d":"EJR4ywPEpo08A10s8Eq8MGIRtmQx_D6szqYKGOl7jSpY",'
        b'"i":"1AABAg299p5IMvuw71HW_TlbzGq5cVOQ7bRbeDuhheF-DPYk","s":"0","kt":1,'
        b'"k":["1AABAg299p5IMvuw71HW_TlbzGq5cVOQ7bRbeDuhheF-DPYk"],'
        b'"nt":1,"n":["EJ6Ycs7kho8XRxiq3DK37jiJ8mU9RP9HpSYnARm26EnO"],'
        b'"bt":0,"b":[],"c":[],"a":[]}'
    ...
    assert serder0.ked["bt"] == "0"  # hex str
    assert serder0.raw == (
        b'{"v":"KERI10JSON000133_","t":"icp","d":"EJZHJo2S1oESDhtadHN-RUV4J3DM9xlWHdsi2TI8ztZ6",'
        b'"i":"1AABAg299p5IMvuw71HW_TlbzGq5cVOQ7bRbeDuhheF-DPYk","s":"0","kt":"1",'
        b'"k":["1AABAg299p5IMvuw71HW_TlbzGq5cVOQ7bRbeDuhheF-DPYk"],'
        b'"nt":"1","n":["EJ6Ycs7kho8XRxiq3DK37jiJ8mU9RP9HpSYnARm26EnO"],'
        b'"bt":"0","b":[],"c":[],"a":[]}'
    ...
    signer2 = Signer(
        raw=seed2, code=MtrDex.ECDSA_256k1_Seed
    )  # next signing keypair transferable is default
    ...
    assert serder1.ked["bt"] == "0"  # hex str
    assert serder1.raw == (
        b'{"v":"KERI10JSON000168_","t":"rot","d":"EKpAPAgfGLxzXuusVQJ4uTuSOzt1mm3a8K1VPRnJOufJ",'
    ...
    assert serder1.ked["bt"] == 0
    assert serder1.raw == (
        b'{"v":"KERI10JSON000162_","t":"rot","d":"EHc84kDs5EsLQYVLkP7fe-7DUfCQ7jFY69Zqq2UfmvTe",'
        b'"i":"1AABAg299p5IMvuw71HW_TlbzGq5cVOQ7bRbeDuhheF-DPYk","s":"1","p":"EJZHJo2S1oESDhtadHN-RUV4J3DM9xlWHdsi2TI8ztZ6",'
        b'"kt":1,"k":["1AABA7KZA_wxPCXJ5BgZ9jjdrMIy3OQKgHfa6eKyLcZpEn26"],'
        b'"nt":1,"n":["EDn6z-KqmwcDVCql1CkMkvSNbNghhMF2TwsdllyP4a07"],'
        b'"bt":0,"br":[],"ba":[],"a":[]}'
    ...
    assert serder2.ked["p"] == serder1.said
    assert serder2.raw == (
        b'{"v":"KERI10JSON0000cf_","t":"ixn","d":"EGa_yUkSwFvJTbFnEBosvpIkJ_AkyY5E3XdU7fqhtErf",'
    ...
    assert serder3.ked["d"] == serder2.said
    assert serder3.raw == (
        b'{"v":"KERI10JSON000095_","t":"rct","d":"EGa_yUkSwFvJTbFnEBosvpIkJ_AkyY5E3XdU7fqhtErf",'
    ...
    signerD = Signer(
        raw=seedD, code=MtrDex.ECDSA_256k1_Seed
    )  # next signing keypair transferable is default
    ...
    assert serderD.ked["n"] == nxtD
    assert serderD.raw == (
        b'{"v":"KERI10JSON000163_","t":"dip","d":"EFVACrfsy2Ke_tjqq-wroc-TE0IFZ-QNwQwuMVzl0rgj",'
    ...
    signerR = Signer(
        raw=seedR, code=MtrDex.ECDSA_256k1_Seed
    )  # next signing keypair transferable is default
    ...
    assert serderR.ked["n"] == nxtR
    assert serderR.raw == (
        b'{"v":"KERI10JSON000164_","t":"drt","d":"EMN4ZdZEZzB0FyHzAOKehHTa6WvvBfK3xwylPuxoJ4sO",'
...
def test_state(mockHelpingNowUTC):
    ...
    eevt = StateEstEvent(
        s="3", d="EUskHI462CuIMS_gNkcl_QewzrRSKH2p9zHQIO132Z30", br=[preW0], ba=[preW3]
    )
    ...
    raw = ksr._asjson()
    assert raw == (
        b'{"vn":[1,0],"i":"DN6WBhWqp6wC08no2iWhgFYTaUgrasnqz6llSvWQTWZN","s":"4","p":"'
    ...
    seal = SealEvent(i=preE, s="0", d="EAuNWHss_H_kH4cG7Li1jn2DXfrEaqN7zhqTEhkeDZ2z")
    ...
    # create namedtuple of latest est event
    eevt = StateEstEvent(
        s="3", d="EAskHI462CuIMS_gNkcl_QewzrRSKH2p9zHQIO132Z30", br=[preW0], ba=[preW3]
    )
...
def test_messagize():
    ...
    with openDB(name="edy") as db, openKS(name="edy") as ks:
        ...
        assert msg == (
            b'{"v":"KERI10JSON0000fd_","t":"icp","d":"EFyzzg2Mp5A3ecChc6AhSLTQ'
        ...
        # create SealEvent for endorsers est evt whose keys use to sign
        seal = SealEvent(
            i="DAvCLRr5luWmp7keDvDuLP0kIqcyBYq79b3Dho1QvrjI",
            s="0",
        ...
        msg = messagize(
            serder, sigers=sigers, cigars=cigars, wigers=wigers, pipelined=True
        )
        assert msg == (
            b'{"v":"KERI10JSON0000fd_","t":"icp","d":"EFyzzg2Mp5A3ecChc6AhSLTQ'
            b'ssBZAmNvPnGxjJyHxl4F","i":"EFyzzg2Mp5A3ecChc6AhSLTQssBZAmNvPnGxj'
        ...
        ked = serder.ked
        reserder = receipt(pre=ked["i"], sn=int(ked["s"], 16), said=serder.said)
        ...
        # Test with cigars
        cigars = mgr.sign(
            ser=serder.raw, verfers=verfers, indexed=False
        )  # sign event not receipt
        msg = messagize(reserder, cigars=cigars)
        assert msg == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EFyzzg2Mp5A3ecChc6AhSLTQ'
            b'ssBZAmNvPnGxjJyHxl4F","i":"EFyzzg2Mp5A3ecChc6AhSLTQssBZAmNvPnGxj'
        ...
        assert msg == (
            b'{"v":"KERI10JSON0000c9_","t":"qry","d":"EGN68_seecuzXQO15FFGJLVw'
...
def test_kever(mockHelpingNowUTC):
    ...
    with openDB() as db:  # Transferable case
        ...
        serder = serdering.SerderKERI(
            makify=True, ilk=Ilks.icp, saids={"i": PreDex.Ed25519}, verify=False
        )
        ...
        assert [key for key in ksr.k] == [verfer.qb64 for verfer in kever.verfers]
        assert ksr._asjson() == (
            b'{"vn":[1,0],"i":"DAUDqkmn-hqlQKD8W-FAEa5JUvJC2I9yarEem-AAEg3e","s":"0","p":"'
    ...
    with openDB() as db:  # Non-Transferable case Error nxt not empty
        ...
        serder = serdering.SerderKERI(
            makify=True, ilk=Ilks.icp, saids={"i": PreDex.Ed25519N}, verify=False
        )
...
def test_keyeventsequence_0():
    ...
    with openDB(name="controller") as conlgr:
        ...
        assert pigers is not None
        assert [
            diger.qb64 for diger in pigers
        ] == nxt2  # digs from rot before rot before ixn
...
def test_multisig_digprefix():
    ...
    with openDB(name="controller") as conlgr, openDB(name="validator") as vallgr:
        ...
        nxtkeys = [
            signers[3].verfer.qb64b,
            signers[4].verfer.qb64b,
            signers[5].verfer.qb64b,
        ]
        sith = "2"
        ...
        count = len(keys)
        counter = Counter(
            Codens.ControllerIdxSigs, count=count, version=Vrsn_1_0
        )  # default is count = 1
        # sign serialization
        ...
        assert msgs == (
            b'{"v":"KERI10JSON0001e7_","t":"icp","d":"EBfxc4RiVY6saIFmUfEtETs1'
        ...
        nxtkeys = [
            signers[5].verfer.qb64b,
            signers[6].verfer.qb64b,
            signers[7].verfer.qb64b,
        ]
        serder = rotate(
        ...
        # Event 2 Interaction
        serder = interact(pre=kever.prefixer.qb64, dig=kever.serder.said, sn=2)
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, count=count, version=Vrsn_1_0
        )  # default is count = 1
        ...
        # Event 4 Interaction
        serder = interact(pre=kever.prefixer.qb64, dig=kever.serder.said, sn=3)
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, count=count, version=Vrsn_1_0
        )  # default is count = 1
        ...
        serder = rotate(
            pre=kever.prefixer.qb64, keys=keys, isith="2", dig=kever.serder.said, sn=4
        )
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, count=count, version=Vrsn_1_0
...
def test_recovery():
    ...
    with openDB(name="controller") as conlgr, openDB(name="validator") as vallgr:
        ...
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, version=Vrsn_1_0
        )  # default is count = 1
        # sign serialization
        ...
        assert sn == 2
        assert esn == 1
        serder = interact(pre=kever.prefixer.qb64, dig=kever.serder.said, sn=sn)
        event_digs.append(serder.said)
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, version=Vrsn_1_0
        ...
        db_est_digs = [val for val in kever.db.kels.getOnLastIter(keys=pre)]
        ...
        assert vkever.sn == kever.sn
        assert (
            vkever.verfers[0].qb64 == kever.verfers[0].qb64 == signers[esn].verfer.qb64
        )

        y_db_digs = [val for val in kevery.db.kels.getOnIterAll(keys=pre)]
        assert db_digs == y_db_digs
        y_db_est_digs = [val for val in kevery.db.kels.getOnLastIter(keys=pre)]
...
def test_receipt():
    ...
    with openDB(name="controller") as coeLogger, openDB(name="validator") as valLogger:
        ...
        reserder = receipt(
            pre=coeKever.prefixer.qb64, sn=coeKever.sn, said=coeKever.serder.said
        )
        # sign event not receipt
        valCigar = valSigner.sign(ser=serder.raw)  # returns Cigar cause no index
        assert valCigar.qb64 == (
            "0BADE2aOlwLi6OCF-jzRWSPuaOo916ADjwhA92hBQ1km"
        ...
        assert res == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EK5yV6IiXCZ-EXkFrBDhrf0I'
        ...
        result = coeKevery.db.rcts.get(
            keys=(coeKever.prefixer.qb64, coeKever.serder.said)
        )
        ...
        reserder = receipt(pre=coeKever.prefixer.qb64, sn=2, said=fake)
        # sign event not receipt
        valCigar = valSigner.sign(ser=serder.raw)  # returns Cigar cause no index
        recnt = Counter(code=Codens.NonTransReceiptCouples, count=1, version=Vrsn_1_0)
        ...
        result = coeKevery.db.ures.get(
            keys=(coeKever.prefixer.qb64, Number(num=2, code=NumDex.Huge).qb64)
        )
        ...
        reserder = receipt(pre=coeKever.prefixer.qb64, sn=coeKever.sn, said=fake)
        # sign event not receipt
        valCigar = valSigner.sign(ser=serder.raw)  # returns Cigar cause no index
        recnt = Counter(code=Codens.NonTransReceiptCouples, count=1, version=Vrsn_1_0)
        # attach to receipt msg stream
        ...
        assert sn == 2
        assert esn == 1
        serder = interact(pre=coeKever.prefixer.qb64, dig=coeKever.serder.said, sn=sn)
        event_digs.append(serder.said)
        # create sig counter
        counter = Counter(
            Codens.ControllerIdxSigs, version=Vrsn_1_0
        ...
        db_digs = [val for val in coeKever.db.kels.getOnIterAll(keys=coepre)]
        assert len(db_digs) == len(event_digs) == 7

        assert valKever.sn == coeKever.sn
        assert (
            valKever.verfers[0].qb64
            == coeKever.verfers[0].qb64
            == coeSigners[esn].verfer.qb64
        )
...
def test_process_attached_receipt_couples_firner_missing_fels():
    ...
    serder = incept(
        keys=[signer.verfer.qb64], ndigs=[Diger(ser=signer.verfer.qb64b).qb64]
    )
...
def test_direct_mode():
    ...
    with openDB(name="controller") as coeLogger, openDB(name="validator") as valLogger:
        ...
        assert cmsg == (
            b'{"v":"KERI10JSON00012b_","t":"icp","d":"EJe_sKQb1otKrz6COIL8VFvB'
        ...
        valSerder = incept(
            keys=[valSigners[vesn].verfer.qb64],
        ...
        assert vmsg == (
            b'{"v":"KERI10JSON00012b_","t":"icp","d":"EAzjKx3hSVJArKpIOVt2KfTR'
        ...
        seal = SealEvent(
            i=valpre, s="{:x}".format(valKever.lastEst.s), d=valKever.lastEst.d
        )
        coeK = valKevery.kevers[coepre]  # lookup coeKever from val's .kevers
        # create validator receipt
        reserder = receipt(pre=coeK.prefixer.qb64, sn=coeK.sn, said=coeK.serder.said)
        ...
        assert (
            coeIcpDig
            == coeK.serder.saidb
            == b"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn1vm0IlL"
        )
        s = valKevery.db.evts.get(keys=(coepre, coeIcpDig))
        assert s.raw == (
            b'{"v":"KERI10JSON00012b_","t":"icp","d":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFG'
        ...
        assert siger.qb64 == (
            "AAD-iI61odpZQjzm0fN9ZATjHx-KjQ9W3-CIlvhowwUaPC5K"
            "nQAIGYFuWJyRgAQalYVSEWoyMK2id_ONTFUE-NcF"
        )
        rmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert rmsg == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EJe_sKQb1otKrz6COIL8VFvB'
            b'v3DEFvtKaVFGn1vm0IlL","i":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn'
        ...
        result = coeKevery.db.vrcs.get(
            keys=dgKey(pre=coeKever.prefixer.qb64, dig=coeKever.serder.said)
        )
        ...
        fake = reserder.said  # some other dig
        reserder = receipt(pre=coeK.prefixer.qb64, sn=10, said=fake)
        # sign event not receipt
        siger = valSigners[vesn].sign(ser=s.raw, index=0)  # return Siger if index

        # create message
        vmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert vmsg == bytearray(
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EJe_sKQb1otKrz6COIL8VFvB'
            b'v3DEFvtKaVFGn1vm0IlL","i":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn'
        ...
        result = coeKevery.db.vres.get(keys=snKey(pre=coeKever.prefixer.qb64, sn=10))
        ...
        seal = SealEvent(
            i=coepre, s="{:x}".format(coeKever.lastEst.s), d=coeKever.lastEst.d
        )
        valK = coeKevery.kevers[valpre]  # lookup valKever from coe's .kevers
        # create validator receipt
        reserder = receipt(pre=valK.prefixer.qb64, sn=valK.sn, said=valK.serder.said)
        ...
        assert (
            valIcpDig
            == valK.serder.saidb
            == b"EAzjKx3hSVJArKpIOVt2KfTRjq8st22hL25Ho9vnNodz"
        )
        s = coeKevery.db.evts.get(keys=(valpre, valIcpDig))
        assert s.raw == (
            b'{"v":"KERI10JSON00012b_","t":"icp","d":"EAzjKx3hSVJArKpIOVt2KfTRjq8st22hL25H'
        ...
        assert siger.qb64 == (
            "AACRmy9_dCMi45BSI89fGeM_ktOTWQctSGrVsZtQMm1RtJZY"
            "31xaNoEN-GJ0c5UrNbNuSyT-wkeit0AeYsPWLEYG"
        )
        # create receipt message
        cmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert cmsg == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EAzjKx3hSVJArKpIOVt2KfTR'
            b'jq8st22hL25Ho9vnNodz","i":"EAzjKx3hSVJArKpIOVt2KfTRjq8st22hL25Ho'
        ...
        result = valKevery.db.vrcs.get(
            keys=dgKey(pre=valKever.prefixer.qb64, dig=valKever.serder.said)
        )
        rctPrefixer, rctNum, rctDiger, rctSiger = result[0]
        ...
        cmsg.extend(siger.qb64b)
        assert cmsg == (
            b'{"v":"KERI10JSON000160_","t":"rot","d":"EKlC013XEpwYuCQ84aVnEAqz'
        ...
        assert (
            coeRotDig
            == coeK.serder.saidb
            == b"EKlC013XEpwYuCQ84aVnEAqzNurjAJDN6ayK-9NxggAr"
        )
        s = valKevery.db.evts.get(keys=(coepre, coeRotDig))
        assert s.raw == (
            b'{"v":"KERI10JSON000160_","t":"rot","d":"EKlC013XEpwYuCQ84aVnEAqzNurjAJDN6ayK'
        ...
        siger = valSigners[vesn].sign(ser=s.raw, index=0)  # return Siger if index
        assert siger.qb64 == (
            "AAANSIICz13kvy4hk2bvTCr2b2uePn4uTf4_nwdolkI77Voq"
            "sm5QFtF6z6sjJK7_oTLY36k2VigSExx0UgGQV7YL"
        )
        # val create receipt message
        vmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert vmsg == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EKlC013XEpwYuCQ84aVnEAqz'
            b'NurjAJDN6ayK-9NxggAr","i":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn'
        ...
        assert cesn == 1
        coeSerder = interact(
            pre=coeKever.prefixer.qb64, dig=coeKever.serder.said, sn=csn
        )
        coe_event_digs.append(coeSerder.said)
        # create sig counter
        counter = Counter(
        ...
        cmsg.extend(siger.qb64b)
        assert cmsg == (
            b'{"v":"KERI10JSON0000cb_","t":"ixn","d":"EG3O9AV3lhySOadwTn810vHO'
        ...
        assert (
            coeIxnDig
            == coeK.serder.saidb
            == b"EG3O9AV3lhySOadwTn810vHOZDc6B8TZY_u_4_iy_ono"
        )
        s = valKevery.db.evts.get(keys=(coepre, coeIxnDig))
        assert s.raw == (
            b'{"v":"KERI10JSON0000cb_","t":"ixn","d":"EG3O9AV3lhySOadwTn810vHOZDc6B8TZY_u_'
            b'4_iy_ono","i":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn1vm0IlL","s":"2","p":"EK'
            b'lC013XEpwYuCQ84aVnEAqzNurjAJDN6ayK-9NxggAr","a":[]}'
        )
        siger = valSigners[vesn].sign(ser=s.raw, index=0)  # return Siger if index
        assert siger.qb64 == (
            "AABP_iABSPKxN2_pcedeIu1qb9rIj5nLaGaiPOW2BFSUQQ7C"
            "SL9IW1s9_wVAxv2idySMjiGuLOZk8qI2thqMZ3ED"
        )
        # create receipt message
        vmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert vmsg == (
            b'{"v":"KERI10JSON000091_","t":"rct","d":"EG3O9AV3lhySOadwTn810vHO'
            b'ZDc6B8TZY_u_4_iy_ono","i":"EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn'
        ...
        assert (
            ...
            == coe_event_digs
            == [
                "EJe_sKQb1otKrz6COIL8VFvBv3DEFvtKaVFGn1vm0IlL",
        ...
        assert (
            db_digs
            == val_event_digs
            == ["EAzjKx3hSVJArKpIOVt2KfTRjq8st22hL25Ho9vnNodz"]
        )
...
def test_direct_mode_cbor_mgpk():
    ...
    with openDB(name="controller") as coeLogger, openDB(name="validator") as valLogger:
        ...
        cmsg.extend(siger.qb64b)
        assert cmsg == (
            b"\xadavqKERI10CBOR0000f9_atcicpadx,EDTOWE_oHAO7j6rhUMGfQ_kX8GJbpaAhO"
        ...
        vmsg.extend(siger.qb64b)
        assert vmsg == (
            b"\x8d\xa1v\xb1KERI10MGPK0000f9_\xa1t\xa3icp\xa1d\xd9,EFBYcX4vOeL7Y"
        ...
        # create validator receipt
        reserder = receipt(
            pre=coeK.prefixer.qb64, sn=coeK.sn, said=coeK.serder.said, kind=Kinds.mgpk
        )
        # sign coe's event not receipt
        # look up event to sign from val's kever for coe
        coeIcpDig = valKevery.db.kels.getOnLast(keys=coepre, on=csn)
        ...
        s = valKevery.db.evts.get(keys=(coepre, coeIcpDig))
        assert s.raw == (
            b"\xadavqKERI10CBOR0000f9_atcicpadx,EDTOWE_oHAO7j6rhUMGfQ_kX8GJbpaAhO-luqqsp5"
        ...
        rmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert rmsg == (
            b"\x85\xa1v\xb1KERI10MGPK00007f_\xa1t\xa3rct\xa1d\xd9,EDTOWE_oHAO7j"
        ...
        fake = reserder.said  # some other dig
        reserder = receipt(pre=coeK.prefixer.qb64, sn=10, said=fake, kind=Kinds.mgpk)
        # sign event not receipt
        siger = valSigners[vesn].sign(ser=s.raw, index=0)  # return Siger if index
        ...
        # create validator receipt
        reserder = receipt(
            pre=valK.prefixer.qb64, sn=valK.sn, said=valK.serder.said, kind=Kinds.cbor
        )
        # sign vals's event not receipt
        # look up event to sign from coe's kever for val
        valIcpDig = coeKevery.db.kels.getOnLast(keys=valpre, on=vsn)
        ...
        s = coeKevery.db.evts.get(keys=(valpre, valIcpDig))
        assert s.raw == (
            b"\x8d\xa1v\xb1KERI10MGPK0000f9_\xa1t\xa3icp\xa1d\xd9,EFBYcX4vOeL7Y5pz0iQ5y"
        ...
        cmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert cmsg == (
            b"\xa5avqKERI10CBOR00007f_atcrctadx,EFBYcX4vOeL7Y5pz0iQ5yCfxd19R1dgA_"
        ...
        cmsg.extend(siger.qb64b)
        assert cmsg == (
            b"\xaeavqKERI10CBOR00012b_atcrotadx,EN4m9YLkeBgWVIvwmj45_qdnBBBY61NVZ"
        ...
        s = valKevery.db.evts.get(keys=(coepre, coeRotDig))
        assert s.raw == (
            b"\xaeavqKERI10CBOR00012b_atcrotadx,EN4m9YLkeBgWVIvwmj45_qdnBBBY61NVZbwOe__MA"
        ...
        vmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert vmsg == (
            b"\x85\xa1v\xb1KERI10MGPK00007f_\xa1t\xa3rct\xa1d\xd9,EN4m9YLkeBgWV"
        ...
        cmsg.extend(siger.qb64b)
        assert cmsg == (
            b"\xa7avqKERI10CBOR0000b2_atcixnadx,EEobyRfni6TAnEROE5yL9sC6lhKEbpbmX"
        ...
        s = valKevery.db.evts.get(keys=(coepre, coeIxnDig))
        assert s.raw == (
            b"\xa7avqKERI10CBOR0000b2_atcixnadx,EEobyRfni6TAn"
        ...
        vmsg = messagize(serder=reserder, sigers=[siger], seal=seal)
        assert vmsg == (
            b"\x85\xa1v\xb1KERI10MGPK00007f_\xa1t\xa3rct\xa1d\xd9,EEobyRfni6TAn"
        ...
        assert (
            ...
            == coe_event_digs
            == [
                "EDTOWE_oHAO7j6rhUMGfQ_kX8GJbpaAhO-luqqsp5mK-",
        ...
        assert (
            db_digs
            == val_event_digs
            == ["EFBYcX4vOeL7Y5pz0iQ5yCfxd19R1dgA_r9i1nVdqMZX"]
        )
...
def test_process_transferable():
    ...
    ked0 = dict(
        v=versify(kind=Kinds.json, size=0),  # version string
        t=Ilks.icp,
        d="",  # SAID
        i="",  # qb64 prefix
        s="{:x}".format(sn),  # hex string no leading zeros lowercase
...
def test_process_manual():
    ...
    assert (
        verkey
        == b"\xaf\x96\xb0p\xfb0\xa7\xd0\xa4\x18\xc9\xdc\x1d\x86\xc2:\x98\xf7?t\x1b\xde.\xcc\xcb;\x8a\xb0"
        b"\xa2O\xe7K"
    )
    ...
    assert (
        verkey
        == b"\xf5DOB:<\xcd\x16\x18\x9b\x83L\xa5\x0c\x98X\x90C\x1a\xb30O\xa5\x0f\xe39l\xa6\xdfX\x185"
    )
    assert len(verkey) == 32
    ...
    assert nxtdig == (
        b"7\x16$m\xb0oA\x171\x94\xf3\xb3\x80\xe03\x00\x167\xf4'\xd8N\xa82D\xed_`"
        b"\xbc\x13\xfe\x11"
    )
    ...
    assert txsrdr.raw == (
        b'{"v":"KERI10JSON00012b_","t":"icp","d":"EKYHED-wvkYDZv4tNUF9qiC1kgnnGLS9YUU8'
    ...
    sig0raw = pysodium.crypto_sign_detached(
        txsrdr.raw, aidseed + aidmat.raw
    )  # sigkey = seed + verkey
    ...
    assert (
        txsigmat.qb64
        == "AAAbwxAS2DuLS4HLaiv9hHd6YEolpPjMKYRdXY3VwmWKh5gW5J2QBsZ1cMmdrlyIlgvqTQEViicNv9mHHOvj9ZAB"
    )
    ...
    result = pysodium.crypto_sign_verify_detached(
        rxsigmat.raw, rxsrdr.raw, rxvermat.raw
    )
    assert not result  # None if verifies successfully else raises ValueError
    """ Done Test """
...
def test_reload_kever(mockHelpingNowUTC):
    ...
    with habbing.openHby(
        name="nat", base="test", salt=Salter(raw=b"0123456789abcdef").qb64
    ) as natHby:
        ...
        assert natHab.db.path.endswith(
            os.path.join(os.path.sep, "keri", "db", "test", "nat")
        )
        ...
        assert (
            natHab.kever.serder.said == "EA3QbTpV15MvLSXHSedm4lRYdQhmYXqXafsD4i75B_yo"
        )
        ...
        assert state._asjson() == (
            b'{"vn":[1,0],"i":"EBm9JqQKS4a3EYv5I7BmAPiwhdSQvFAOpqe0dgk3kgH_","s":"6","p":"'
...
def test_load_event(mockHelpingNowUTC):
    with (
        habbing.openHby(
            name="tor", base="test", salt=Salter(raw=b"0123456789abcdef").qb64
        ) as torHby,
        habbing.openHby(
            name="wil", base="test", salt=Salter(raw=b"0123456789abcdef").qb64
        ) as wilHby,
        habbing.openHby(
            name="wan", base="test", salt=Salter(raw=b"0123456789abcdef").qb64
        ) as wanHby,
        habbing.openHby(
            name="tee", base="test", salt=Salter(raw=b"0123456789abcdef").qb64
        ) as teeHby,
    ):
        ...
        parsing.Parser(version=Vrsn_1_0).parse(
            ims=bytearray(torIcp), kvy=wanHab.kvy, local=True
        )  # process as local
        ...
        ixn = torHab.interact(
            data=[dict(i=teeHab.pre, s="0", d=teeHab.kever.serder.said)]
        )
        parsing.Parser(version=Vrsn_1_0).parse(
            ims=bytearray(ixn), kvy=wanHab.kvy, local=True
        )  # give to wan must be local
        wanHab.processCues(wanHab.kvy.cues)  # process cue returns rct msg
        ...
        parsing.Parser(version=Vrsn_1_0).parse(
            ims=bytearray(teeIcp), kvy=wanKvy, local=True
        )  # local
        parsing.Parser(version=Vrsn_1_0).parse(
            ims=bytearray(rct), kvy=wanHab.kvy, local=True
        )  # local
        parsing.Parser(version=Vrsn_1_0).parse(
            ims=bytearray(nrct), kvy=wanHab.kvy, local=True
        )  # local
        ...
        assert evt == {
            ...
            "receipts": {
                "nontransferable": [
                    {
                        "prefix": "BEXrSXVksXpnfno_Di6RBX2Lsr9VWRAihjLhowfjNOQQ",
                        "signature": "0BCQOeNT3mwAHxh6mYU9K_B2VmbtjJh7_8115k4JrBPR3c4"
                        "3jUSO197H2J73vWMi61qzOovNkSWQbnRx3NFnrk8I",
                    }
                ],
                "transferable": [
                    {
                        "prefix": "EBOVJXs0trI76PRfvJB2fsZ56PrtyR6HrUT9LOBra8VP",
                        "said": "EBOVJXs0trI76PRfvJB2fsZ56PrtyR6HrUT9LOBra8VP",
                        "sequence": "0AAAAAAAAAAAAAAAAAAAAAAA",
                        "signature": "AADGbcmUNw_SX7OVNX-PQYl41UZx_pgJXHOoMWrcfmCDGgkc1-"
                        "MqXJjMD9S9moJ-lpPL9-AiXgITemMZL_QYGzIA",
