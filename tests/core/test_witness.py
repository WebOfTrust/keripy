...
def test_indexed_witness_replay():
    ...
    with openHby(name="cam", base="test", salt=salt) as camHby, \
        ...
        for msg in rctMsgs:  # process rct msgs from all witnesses
            Parser(version=Vrsn_1_0).parse(ims=bytearray(msg), kvy=camKvy, local=True)
        ...
        wigers = camHab.db.wigs.get(keys=(camHab.pre, camHab.kever.serder.said))
        assert len(wigers) == 3
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            Parser(version=Vrsn_1_0).parse(ims=bytearray(camIcpWitRctMsg), kvy=kvy, local=True)
            assert len(kvy.db.wigs.get(keys=(camHab.pre, camHab.kever.serder.said))) == 3  # fully witnessed
            assert len(kvy.cues) == 0  # no cues

        # send Cam icp and witness rcts to Van
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camIcpMsg), kvy=vanKvy, local=True)
        # should escrow since not witnesses
        assert camHab.pre not in vanKvy.kevers
        # process receipts
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camIcpWitRctMsg), kvy=vanKvy, local=True)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            Parser(version=Vrsn_1_0).parse(ims=bytearray(camIxnWitRctMsg), kvy=kvy, local=True)
            assert len(kvy.db.wigs.get(keys=(camHab.pre, camHab.kever.serder.said))) == 3  # fully witnessed
            assert len(kvy.cues) == 0  # no cues

        # send Cam ixn's witness rcts to Van first then send Cam ixn
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camIxnWitRctMsg), kvy=vanKvy, local=True)
        vanKvy.processEscrows()
        assert vcKvr.sn == 0
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camIxnMsg), kvy=vanKvy, local=True)
        assert vcKvr.sn == 0
        vanKvy.processEscrows()
        assert vcKvr.sn == 1
        ...
        Parser(version=Vrsn_1_0).parse(ims=bytearray(wilRctMsg), kvy=camKvy, local=True)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            Parser(version=Vrsn_1_0).parse(ims=bytearray(camRotMsg), kvy=kvy, local=True)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            Parser(version=Vrsn_1_0).parse(ims=bytearray(camRotWitRctMsg), kvy=kvy, local=True)
        ...
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camRotWitRctMsg), kvy=vanKvy, local=True)
        vanKvy.processEscrows()
        assert vcKvr.sn == 1
        Parser(version=Vrsn_1_0).parse(ims=bytearray(camRotMsg), kvy=vanKvy, local=True)
        assert vcKvr.sn == 1
        vanKvy.processEscrows()
        assert vcKvr.sn == 2
        assert vcKvr.wits == camHab.kever.wits
...
def test_nonindexed_witness_receipts():
    ...
    with openHby(name="cam", base="test", salt=salt) as camHby, \
        ...
        for i, msg in enumerate(rctMsgs):
            Parser(version=Vrsn_1_0).parse(ims=bytearray(msg), kvy=vanKvy, local=True)
            # escrows to Ure
            assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == i + 1  # escrows
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == len(rctMsgs)  # all in escrow
        ...
        assert vanKvy.db.pwes.cntAll(keys=camHab.pre, on=camHab.kever.serder.sn) == 0  # nothing in partial witness escrow
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == len(rctMsgs)  # still in escrow
        assert vanKvy.db.wigs.cnt(keys=(camHab.pre, camHab.kever.serder.said)) == 0  # no wigs yet
        ...
        assert vanKvy.db.pwes.cntAll(keys=camHab.pre, on=camHab.kever.serder.sn) == 1  # now in partial witness escrow
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == len(rctMsgs)  # still in escrow
        assert vanKvy.db.wigs.cnt(keys=(camHab.pre, camHab.kever.serder.said)) == 0  # no wigs yet
        assert camHab.pre not in vanKvy.kevers  # not accepted

        vanKvy.processEscrows()  # process escrows
        assert vanKvy.db.pwes.cntAll(keys=camHab.pre, on=camHab.kever.serder.sn) == 0  # unescrowed from first stage
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == 0  # out of first stage
        ...
        Parser(version=Vrsn_1_0).parse(ims=bytearray(wilRctMsg), kvy=vanKvy, local=True)

        assert vanKvy.db.ures.cnt(keys=(camHab.pre, Number(num=camHab.kever.serder.sn, code=NumDex.Huge).qb64)) == len(rctMsgs)  # no change
...
def test_out_of_order_witnessed_events():
    ...
    with openHby(name="wes", base="test", salt=default_salt) as wesHby, \
        ...
        oodig = bamHby.db.ooes.get(keys=bobHab.pre.encode("utf-8"), on=1)
        ...
        pwedig = bamHby.db.pwes.get(keys=bobHab.pre.encode("utf-8"), on=1)
        assert pwedig == []
