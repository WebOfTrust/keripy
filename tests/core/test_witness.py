...
from keri.core import Salter, eventing, parsing, serdering, coring
...
def test_nonindexed_witness_receipts():
    ...
    with habbing.openHby(name="cam", base="test", salt=salt) as camHby, \
        ...
        for i, msg in enumerate(rctMsgs):
            ...
            # escrows to Ure
            assert vanKvy.db.ures.cnt(keys=(camHab.pre, coring.Number(num=camHab.kever.serder.sn, code=coring.NumDex.Huge).qb64)) == i + 1  # escrows
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, coring.Number(num=camHab.kever.serder.sn, code=coring.NumDex.Huge).qb64)) == len(rctMsgs)  # all in escrow
        ...
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, coring.Number(num=camHab.kever.serder.sn, code=coring.NumDex.Huge).qb64)) == 0  # out of first stage
        ...
        assert vanKvy.db.ures.cnt(keys=(camHab.pre, coring.Number(num=camHab.kever.serder.sn, code=coring.NumDex.Huge).qb64)) == len(rctMsgs)  # no change
