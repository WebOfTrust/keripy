...
def test_witness():
    ...
    with basing.openDB(name="cam") as camDB, keeping.openKS(name="cam") as camKS, \
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camIcpMsg), kvy=kvy)
        ...
        for msg in rctMsgs:# process rct msgs from all witnesses
            parsing.Parser().process(ims=bytearray(msg), kvy=camKvy)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camIcpWitRctMsg), kvy=kvy)
        ...
        parsing.Parser().process(ims=bytearray(camIcpMsg), kvy=vanKvy)
        # vanKvy.process(ims=bytearray(camIcpMsg))  # should escrow since not witnesses
        assert camHab.pre not in vanKvy.kevers
        parsing.Parser().process(ims=bytearray(camIcpWitRctMsg), kvy=vanKvy)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camIxnMsg), kvy=kvy)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camIxnWitRctMsg), kvy=kvy)
        ...
        parsing.Parser().process(ims=bytearray(camIxnWitRctMsg), kvy=vanKvy)
        # vanKvy.process(ims=bytearray(camIxnWitRctMsg))
        vanKvy.processEscrows()
        assert vcKvr.sn == 0
        parsing.Parser().process(ims=bytearray(camIxnMsg), kvy=vanKvy)
        ...
        parsing.Parser().process(ims=bytearray(camReplayMsg), kvy=wilKvy)
        ...
        parsing.Parser().process(ims=bytearray(wilRctMsg), kvy=camKvy)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camRotMsg), kvy=kvy)
        ...
        for i in range(len(camWitKvys)):
            kvy = camWitKvys[i]
            parsing.Parser().process(ims=bytearray(camRotWitRctMsg), kvy=kvy)
        ...
        parsing.Parser().process(ims=bytearray(camRotWitRctMsg), kvy=vanKvy)
        # vanKvy.process(ims=bytearray(camRotWitRctMsg))
        vanKvy.processEscrows()
        assert vcKvr.sn == 1
        parsing.Parser().process(ims=bytearray(camRotMsg), kvy=vanKvy)
        # vanKvy.process(ims=bytearray(camRotMsg))  # should escrow since not witnesses
        assert vcKvr.sn == 1
        vanKvy.processEscrows()
        assert vcKvr.sn == 2
