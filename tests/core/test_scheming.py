...
from keri.core.scheming import Schemer, SchemaTypes, Saider
...
def test_json_schema_dict():
    ...
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.Blake3_256)
    assert sce.said == "EQtF_DhWj-uCPTsq4BONO0yR0PWLpUITkSqHoW0JjndY"
    assert sce.verify(raw=payload) is True
    assert sce.verify(raw=mismatch) is False
    assert sce.verify(raw=badjson) is False
    ...
    with pytest.raises(ValidationError):
        Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.Blake3_256)
    ...
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.Blake2b_256)
    assert sce.said == "FtDGHM1-15UdUiIki8mtKgfpC9CLxuq16wr55nw3htZs"
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.Blake2s_256)
    assert sce.said == "Ga063NUEvWf4ZNm7qgbAUVhcBMEL8vddQhEauTk2HuKo"
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.SHA3_256)
    assert sce.said == "HUhsiL9Hl9c6DVZ9YioCKiyLIuJmnEd0ALKNge3bMMn0"
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.SHA3_512)
    assert sce.said == "0FaBVrb4jLlfgSqNaAGf3gFpRJJvZUiyPg-2W240y3IO1SCv2kD3rkowQ9i9yOVYT_K3BZ54eBN1zpqvkoEMk7YQ"
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.SHA2_256)
    assert sce.said == "IRgVJumn8RaXh0WJV1QT3zS3rUwhuCJDDhzgFRE-Fdn4"
    sce = Schemer(sed=sed, kind=SchemaTypes.JSONSchema, code=MtrDex.SHA2_512)
