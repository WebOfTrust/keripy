...
def test_ipex_v2_anchored_flows_through_kram(fakeHelpingClock):
    ...
    with (openHby(name="ipex-v2-anchored-kram-grantor",
        ...
        with (openCF(name="ipex-v2-anchored-kram-grantor",
            ...
            # couple in the durable exchange-source store.
            assert applicantHby.db.esrc.cnt(keys=(grantExn.said,)) == 1
            ...
            roundTrip = Parser(version=Vrsn_2_0).parse(
                ...
                framed=False,
                processive=False)[0]
            # Serialization must preserve both fields of the source couple.
            assert len(roundTrip.sscs) == 1
            assert roundTrip.sscs[0][0].sn == grantResult.sscs[0][0].sn
            assert roundTrip.sscs[0][1].qb64 == grantResult.sscs[0][1].qb64
...
def test_ipex_v2_anchored_grant_waits_for_sender_kel_event(fakeHelpingClock):
    ...
    with (openHby(name="ipex-v2-anchor-order-issuer",
        ...
        with openCF(name="ipex-v2-anchor-order", base="test", temp=True) as cf:
            ...
            assert recipientHby.db.esrc.cnt(keys=(grantExn.said,)) == 1
            assert list(exc.cues) == [
                dict(kin="query",
                     q=dict(r="logs", pre=issuerHab.pre,
                            sn=Number(num=anchorSn).snh)),
            ]
...
def test_ipex_v2_escrows_registry_backed_grant_until_tel_evidence_arrives():
    ...
    with (openHby(name="ipex-v2-proof-escrow-issuer",
        ...
        try:
            ...
            # The source event is part of the sender's normal KEL publication
            # flow and must arrive before the anchored exchange.
            grantAnchor = issuerHab.msgOwnEvent(sn=issuerHab.kever.sn,
                                                framed=True,
                                                gvrsn=Vrsn_2_0)
            Parser(version=Vrsn_2_0).parse(ims=bytearray(grantAnchor),
                                           kvy=recipientRemoteKvy)

            # A registry-backed grant with truthy ax still fails immediately
            # when its direct sender-KEL source couple is removed. It must not
            # fall through to TEL proof escrow as an alternate anchor mode.
            unsealed = issuerHab.endorse(serder=grantExn,
                                         framed=False,
                                         gvrsn=Vrsn_2_0,
                                         nests=[_nest(_proofed(acdc, issuedBlinder))])
            ims = bytearray(unsealed)
            Parser(version=Vrsn_2_0).parse(ims=ims, framed=False, exc=exc)
            assert ims == bytearray()
            assert recipientHby.db.epse.get(keys=(grantExn.said,)) is None
            assert list(exc.cues) == []
            ...
            serialized = serializeMessage(recipientHby, grantExn.said, framed=True)
            result = Parser(version=Vrsn_2_0).parse(
                ims=bytearray(serialized),
                framed=False,
                processive=False)[0]
