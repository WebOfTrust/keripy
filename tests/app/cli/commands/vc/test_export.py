...
def test_export_credentials_full_sets_related_flags(tmp_path, seeder):
    ...
    # This catches regressions where --full or the all-* flags stop configuring ExportDoer correctly.
    with habbing.openHby(name="export-flags", base=isolatedBase(tmp_path),
                         temp=False, clear=True, salt=SALT) as source:
        doer = None
        try:
...
def test_export_selection_revocation_and_registry_file_branches(tmp_path, seeder, helpers):
    ...
    # Late-joiner import depends on these filters excluding revoked creds by default and scoping registries to alias.
...
def test_export_output_file_and_stdout_paths(tmp_path, capsys, seeder, helpers):
    ...
    # The workflow uses files, while stdout remains the legacy/default CLI behavior.
...
def test_export_output_credential_related_material_and_chain_branches(tmp_path, capsys, seeder, helpers):
    ...
    # This protects the --full bundle shape needed when imported credentials depend on chained source credentials.
