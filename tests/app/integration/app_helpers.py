...
class HabHelpers:
    ...
    def hasDelegables(db: basing.Baser) -> List[Tuple[str, int, bytes]]:
        ...
        dlgs = []
        for (pre, sn), edig in db.delegables.getTopItemIter():
...
class MultisigInceptLeader(doing.DoDoer):
    ...
    def _startCounselor(self):
        ...
        number = core.Number(num=0)
        diger = coring.Diger(qb64=prefixer.qb64)
        self.counselor.start(prefixer=prefixer, number=number, diger=diger, ghab=self.ghab)
    ...
    def _isCounselorComplete(self) -> bool:
        ...
        prefixer = coring.Prefixer(qb64=self.ghab.pre)
        number = core.Number(num=0)
        diger = coring.Diger(qb64=self.ghab.pre)
        return self.counselor.complete(prefixer=prefixer, number=number, diger=diger)
...
class MultisigDelegationApprover(doing.DoDoer):
    ...
    def delegablesEscrowed(self) -> List[Tuple[str, int, bytes]]:
        ...
        return [(pre, sn, edig) for (pre, sn), edig in self.hby.db.delegables.getTopItemIter()]
    ...
    def _startCounselorForReadyDelegations(self):
        ...
        for key in list(self.readyForCounselor.keys()):
            ...
            prefixer = coring.Prefixer(qb64=self.ghab.pre)
            number = core.Number(num=info['ixn_sn'])
            diger = coring.Saider(qb64=info['ixn_said'])
    ...
    def _releaseCompletedDelegations(self):
        ...
        for key in list(self.waitingForComplete.keys()):
            ...
            prefixer = coring.Prefixer(qb64=self.ghab.pre)
            number = core.Number(sn=info['ixn_sn'])

            if not self.counselor.complete(prefixer=prefixer, number=number):
                continue
    ...
    def _setAuthorizerSeal(self, key: Tuple[str, str], info: dict):
        ...
        for dpre, dsn, edig in self.delegablesEscrowed():
            if dpre == pre and dsn == sn:
                ...
                sner = core.Number(num=info['ixn_sn'], code=core.NumDex.Huge)
                saider = coring.Saider(qb64=info['ixn_said'])
                self.hby.db.aess.pin(keys=dgkey, val=(sner, saider))
                break
    ...
    def _getValidDelegableEvent(self, pre: str, edig: bytes) -> serdering.SerderKERI:
        ...
        """
        eserder = self.hby.db.evts.get(keys=(pre, edig))
        if eserder is None:
            return None
        ilk = eserder.sad['t']
    ...
    def _signAndCoordinateAnchor(self, delegate_pre: str, delegate_sn: str, anchor_data: list):
        ...
        prefixer = coring.Prefixer(qb64=self.ghab.pre)
        number = core.Number(sn=ixnser.sn)
        diger = coring.Diger(qb64=ixnser.said)
        self.counselor.start(prefixer=prefixer, number=number, diger=diger, ghab=self.ghab)
