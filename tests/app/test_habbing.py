...
def test_habery():
    ...
    assert hby.mgr.tier == core.Tiers.low
    ...
    # signer = core.Salter(qb64=bran).signer(transferable=False, tier=tier, temp=temp)
    # seed = signer.qb64
    ...
    bran = "MyPasscodeARealSecret"
    with habbing.openHby(bran=bran, salt=core.Salter(raw=b'0123456789abcdef').qb64) as hby:
        assert hby.name == "test"
        assert hby.base == ""
        assert hby.temp
        assert hby.inited
...
def test_make_load_hab_with_habery():
    ...
    with habbing.openHby(salt=core.Salter(raw=b'0123456789abcdef').qb64) as hby:  # default is temp=True on openHab
...
def test_postman_endsfor():
    with habbing.openHby(name="test", temp=True, salt=core.Salter(raw=b'0123456789abcdef').qb64) as hby, \
            habbing.openHby(name="wes", temp=True, salt=core.Salter(raw=b'wess-the-witness').qb64) as wesHby, \
