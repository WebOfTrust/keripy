...
def test_multisig_join(mockHelpingNowUTC):
    with openMultiSig(prefix="test") as ((hby1, ghab1), (hby2, ghab2), (hby3, ghab3)):
        with habbing.openHab(name="test_4", salt=b'0123456789abcdef', transferable=True, temp=True) as (hby4, hab4):
            ...
            parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(icp4), kvy=hby1.kvy, local=True)
            parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(icp4), kvy=hby2.kvy, local=True)
            parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(icp4), kvy=hby3.kvy, local=True)
            ...
            msg = eventing.messagize(serder=serder, sigers=sigers)
            parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(msg), kvy=hby1.kvy, local=True)
            ...
            for msgs in [hab1.replay(), hab2.replay(), hab3.replay()]:
                parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(msgs), kvy=hby4.kvy, local=True)
            parsing.Parser(version=Vrsn_1_0).parse(ims=bytearray(ghab1.replay()), kvy=hby4.kvy, local=True)
