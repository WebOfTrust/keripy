...
def test_counselor():
    ...
    with openHab(name=f"{prefix}_1", salt=salt, transferable=True) as (hby1, hab1), \
        ...
        ghab2 = hby2.makeGroupHab(group=f"{prefix}_group2", mhab=hab2,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab2.makeOwnInception(allowPartiallySigned=True, framed=True)
...
def test_the_seven():
    ...
    with openHab(name=f"{prefix}_1", salt=salt, transferable=True) as (hby1, hab1), \
        ...
        icps = \
        [
            hab1.makeOwnEvent(sn=0, framed=True),
            hab2.makeOwnEvent(sn=0, framed=True),
            hab3.makeOwnEvent(sn=0, framed=True),
            hab4.makeOwnEvent(sn=0, framed=True),
            hab5.makeOwnEvent(sn=0, framed=True),
            hab6.makeOwnEvent(sn=0, framed=True),
            hab7.makeOwnEvent(sn=0, framed=True)
        ]
        ...
        ghab3 = hby3.makeGroupHab(group=f"{prefix}_group3", mhab=hab3,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab3.makeOwnInception(allowPartiallySigned=True, framed=True)
        serd = SerderKERI(raw=bytearray(evt))
        ...
        ghab4 = hby4.makeGroupHab(group=f"{prefix}_group4", mhab=hab4,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab4.makeOwnInception(allowPartiallySigned=True, framed=True)
        serd = SerderKERI(raw=bytearray(evt))
        ...
        ghab5 = hby5.makeGroupHab(group=f"{prefix}_group5", mhab=hab5,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab5.makeOwnInception(allowPartiallySigned=True, framed=True)
        serd = SerderKERI(raw=bytearray(evt))
        ...
        ghab6 = hby6.makeGroupHab(group=f"{prefix}_group6", mhab=hab6,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab6.makeOwnInception(allowPartiallySigned=True, framed=True)
        serd = SerderKERI(raw=bytearray(evt))
        ...
        ghab7 = hby7.makeGroupHab(group=f"{prefix}_group7", mhab=hab7,
                                  smids=smids, rmids=rmids, **inits)
        evt = ghab7.makeOwnInception(allowPartiallySigned=True, framed=True)
        serd = SerderKERI(raw=bytearray(evt))
