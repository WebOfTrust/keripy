...
class MultisigInceptLeader(doing.DoDoer):
    ...
    def recur(self, tyme, deeds=None):
        """
        Main orchestration loop for multisig inception as leader.
        
        Flow:
        1. Create GroupHab and send ICP EXN to followers
        2. Wait for message delivery confirmation
        3. Start Counselor for signature coordination
        4. Wait for Counselor to complete (all signatures collected)
        """
        super(MultisigInceptLeader, self).recur(tyme, deeds=deeds)

        # Create GroupHab if not yet done
    ...
    def _createGroupHabAndNotifyFollowers(self):
        """
        Create the GroupHab and send inception EXN to all followers.
        
        This is the first step where the leader:
        1. Creates the multisig GroupHab with all configuration
        2. Creates a partially signed inception event
        3. Sends a /multisig/icp EXN to all other members
        """
        inits = dict(
            isith=self.isith,
            nsith=self.nsith,
        ...
        # Create partially signed inception
    ...
    def _checkPendingSendsComplete(self) -> bool:
        ...
        Returns:
            True if all sends complete, False if still waiting
        """
    ...
    def _startCounselor(self):
        ...
        The Counselor tracks partial signatures from all members
